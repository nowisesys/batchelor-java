...
/**
 * <p>
 * This class implements the WebServiceInterface providing an SOAP client that is used for
 * connections against Batchelors SOAP web service interface. All remote methods might throw an
 * RemoteException.</p>
 *
 * <p>
 * The URL to use deserves an bit of explanation. It is the local or remote location of an WSDL file
 * that describes the Batchelor SOAP service, not the location of the Batchelor SOAP service or its
 * base URL. An typical example is "http://host.example.com/batchelor/ws/schema/wsdl/?wsdl". Note
 * that the WSDL and the SOAP service itself might be at different locations, it might not even be
 * located at the same host!
 * </p>
 *
 * <p>
...
public class BatchelorSoapClient implements WebServiceInterface {
    ...
    /**
     * This contructor overrides the WSDL location that JAX-WS has generated the service code from.
     * Use this constructor for binding to an volatile Batchelor service (on some host).
     *
    ...
    /**
     * This contructor overrides the WSDL location that JAX-WS has generated the service code from.
     * Use this constructor for binding to an volatile Batchelor service (on some host). The name
    ...
    /**
     * Set the location of the WSDL for the service to consume. This adress is used by the SOAP
     * binding provider.
     *
     * @param url The WSDL location.
     */
    ...
    /**
     * Set the name of the service. This method allows the service name to be overridden. If name is
     * null, then the previous used service name is reset causing the SOAP binding provider to use
     * the service name from the WSDL instead.
     *
     * @param name The service name.
     */
    ...
    public List<EnqueueResult> enqueue(String indata) throws RemoteException {
        ...
        try { // Call Web Service Operation
            ...
            getServicePort().enqueue(indata).forEach((in) -> {
    ...
    public List<QueuedJob> queue(QueueSortResult sort, QueueFilterResult filter) throws RemoteException {
        ...
        if (filter == null) {
            filter = QueueFilterResult.ALL;
        }
        try { // Call Web Service Operation
            List<QueuedJob> list = new ArrayList<>();
            for (se.uu.bmc.it.batchelor.soap.schema.QueuedJob in : getServicePort().queue(sort.getValue(), filter.getValue())) {
     ...
     * the UNIX epoch (00:00:00 UTC on January 1, 1970). The stamp to use is typical obtained by
     * calling getStamp() using the EnqueueResult object returned by enqueue().
     *
    ...
    public List<QueuedJob> watch(int stamp) throws RemoteException {
        ...
        try { // Call Web Service Operation
            List<QueuedJob> list = new ArrayList<>();
            getServicePort().watch(stamp).forEach((in) -> {
                list.add(QueuedJobConverter.createQueuedJob(in));
            });
            return list;
        } catch (Exception ex) {
    ...
    public List<JobIdentity> opendir() throws RemoteException {
        ...
        try { // Call Web Service Operation
            ...
            getServicePort().opendir().forEach((in) -> {
