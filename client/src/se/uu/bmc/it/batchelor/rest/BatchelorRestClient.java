...
/**
 * <p>
 * This class implements the WebServiceInterface providing an RESTful client for interaction with
 * the Batchelor REST web service on a remote system. The REST URI space and its associated HTTP
 * actions (GET, PUT, POST and DELETE) is completely abstacted away within this class.</p>
 *
 * <p>
 * Things to keep in mind using this class is that content handling must be initialized before its
 * first use and only once. Note that the content handler is global for the whole application and
 * its entire lifetime. The following code demonstrates how to create a REST service client,
...
public class BatchelorRestClient implements WebServiceInterface {
    ...
    /**
     * Creates an Batchelor REST client using the url as service address. The address should be a
     * complete URL, i.e. https://localhost/batchelor/ws/rest.
     *
    ...
    /**
     * Creates an Batchelor REST client using the url as service address. The address should be a
     * complete URL, i.e. https://localhost/batchelor/ws/rest. The connection is made thru the
     * proxy.
     *
     * @param url The REST service address.
    ...
    /**
     * Creates an Batchelor REST client using the url as service address. The encoder argument
     * request that server response messages should be encoded using a specific method.
     *
     * @param url The REST service address.
    ...
    /**
     * Request that server response messages from the REST service should be encoded using the
     * encoder type (XML or FOA).
     *
    ...
    /**
     * Return a formatted request URL. The URL is built from the service base URL with the request
     * path and encoding method appended.
     *
    ...
    /**
     * Queues an job for later execution. This version of enqueue reads the job data from the indata
     * string.
     *
    ...
    /**
     * Queues an job for later execution. The job data is read from the indata byte array. This
     * function is an REST API specific extension of the WebServiceInterface.
     *
    ...
    /**
     * Queues an job for later execution. The job data is read from the supplied file. This function
     * is an REST API specific extension of the WebServiceInterface.
     *
    ...
    /**
     * <p>
     * Queues an job for later execution. This function enqueues the job using the input stream as
     * the job data source (indata). For some applications this is a requirement because the indata
     * might be big (maybe up to several gigabytes), so trying to use an byte array or string would
     * lead to an OutOfMemoryException.</p>
     *
     * <p>
    ...
    public List<EnqueueResult> enqueue(InputStream stream) throws RemoteException {
        ...
        try {
            ...
            List<EnqueueResult> list = new ArrayList<>();
            result.getJob().stream().map((job) -> JobObjectConverter.createEnqueueResult(job)).forEachOrdered((data) -> {
                list.add(data);
            });
            return list;
        } catch (IOException e) {
    ...
    /**
     * An helper function for dequeue (remove) one or more jobs. This function is an REST API
     * specific extension to the WebServiceInterface.
     *
    ...
    /**
     * Dequeue all jobs matching the filter (i.e. SUCCESS, ERROR or CRASHED). This function is an
     * REST API specific extension to the WebServiceInterface.
     *
    ...
    public List<QueuedJob> queue(QueueSortResult sort, QueueFilterResult filter) throws RemoteException {
        ...
        try {
            ...
            List<QueuedJob> list = new ArrayList<>();
            result.getJob().stream().map((job) -> {
    ...
    /**
     * Returns a list of queued jobs that's been enqueued after the given timestamp. The timestamp
     * is the number of seconds since the start of the UNIX epoch (00:00:00 UTC on January 1, 1970).
     *
    ...
    public List<JobIdentity> opendir() throws RemoteException {
        ...
        try {
            ...
            List<JobIdentity> list = new ArrayList<>();
            result.getLink().stream().map((link) -> LinkObjectConverter.createJobIdentity(link)).forEachOrdered((ident) -> {
                list.add(ident);
            });
            return list;
        } catch (IOException e) {
     ...
     * se.uu.bmc.it.batchelor.codecs.base64.Base64Decoder class can be used to decode the input
     * stream.
     * </p>
     *
     * <p>
     ...
     * @param timeout Sets the read timeout to a specified timeout, in milliseconds. If the timeout
     * expires before there is data available for read, a java.net.SocketTimeoutException is raised.
     ...
     *
     * <p>
     * This special version of fopen() saves the content of the remote file to a local file (the
     * path argument). This method is an REST API specific extension of the WebServiceInterface.</p>
     *
     * @param job An unique identifier of the queued job.
     * @param file The remote filename.
        ...
        try (OutputStream out = new FileOutputStream(path.getAbsolutePath())) {
            if (remote.getEncoding().compareTo("base64") == 0) {
                ...
                if (remote.getInputStream() != null) {
                    ...
                    byte[] buff = new byte[8192];
                    while (size > read) {
                        long want = size - read > buff.length ? buff.length : size - read;
                        long curr = in.read(buff, 0, (int) want);
                        if (curr == -1) {
                            break;
                        }
