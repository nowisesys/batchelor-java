...
/**
 * <p>
 * Implements the ContentHandlerFactory that returns decoders to decode an REST service response
 * (XML or FOA encoded).</p>
 *
 * <p>
 * A ContentHandlerFactory must be set prior to calling any functions that communicates with the
 * REST service. However, the ContentHandlerFactory is system wide and can only be set once per
 * application. One thing to keep in mind is that content handling is an property of the stream
 * handling within the JVM itself, by register an content handler factory you tell the JVM to call
 * your code instead of its default handlers.</p>
 *
 * <p>
 * Case 1: If your application don't use any other ContentHandlerFactory:</p>
 * <pre>
 ...
 *
 * <p>
 * Case 2: If your application use another ContentHandlerFactory, then either decorate your existing
 * class or create a subclass inheriting it:
 * <pre>
