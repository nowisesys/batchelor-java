 ...
 *
 * <p>
 * Encoding an input stream is an iterative process where encode() gets called in a loop until null
...
public final class Base64Encoder {
    ...
    /**
     * Creates a encoder object suitable for encoding strings and char or byte arrays.
     */
    ...
    /**
     * Creates a encoder object suitable for encoding InputStreams. The size argument is the number
     * of bytes to encode from the input stream in each call to encode.
     *
    ...
    /**
     * Set the number of bytes to encode. Calling this function has no effect unless the input
     * source is a stream, for strings or char/byte arrays the size is instead determined from the
     * input.
     *
     * @param size The number of bytes to encode.
     */
    ...
    /**
     * Get the number of bytes that will be encoded by next call of encode(). This function will
     * also return the number of bytes encoded from last call of encode().
     *
    ...
    /**
     * Get the number of bytes required to hold the result of encoding size number of bytes from the
     * input source.
     *
     * @param size The number of bytes to encode.
    ...
    /**
     * Creates an suitable sized result buffer for encoding the size number of bytes.
     *
    ...
    /**
     * Encode the input stream using current input buffer. Call setSize() prior to calling this
     * function to resize the input buffer. The number of bytes to decode is the same as returned by
     * getSize(), that is BUFFER_SIZE by default.
     *
    ...
    /**
     * Encode the input stream in block of size number of bytes. The actual number of bytes encoded
     * might be smaller if EOF is detected or larger if size is not an multiple of 3 (adjusted
     * internal).
     *
     * @param stream The input stream.
     * @param size The number of bytes to encode.
    ...
    public byte[] encode(InputStream stream, int size) throws IOException {
        ...
        while (size > 0) {
            int read = stream.read(buffer, buffer.length - size, size);
            if (read == -1) {
                break;
            }
            size -= read;
     ...
     *
     * @param single Denotes whether the buffer is internal allocated or not (uses external buffer
     * in single call mode).
     * @return The encoded result.
     */
